...
class Perem(object):
    ...
    def set_example(self):
        ...
        print(f"-----Запись строки c {self.first} по {self.last} start {self.j_s}.{self.i_s} end {self.j_e}.{self.i_e}------")
        local_str = my_str.split()
        for n in range(self.first, self.last + 1):
            ...
            if local_str[n] == "и":
                ...
                answ += str("or ")
            elif (local_str[n] == '+' or local_str[n] == '-' or local_str[n] == '*' or local_str[n] == '/' or local_str[n] == '&' or local_str[n] == '|' or local_str[n] == ')' or local_str[n] == '(' or local_str[n] == ':' or local_str[n] == ',' or local_str[n] == '.' or local_str[n] == '='):
...
def find_end_metok(splited_str):
    ...
    if splited_str[i] == "Конец" and splited_str[i+1] == "меток" and splited_str[global_i_in_splited_str+1] != "Конец" and i != len(splited_str)-2:
        return True
    else:
        return False
                ...
                if operator(splited_str):
                    ...
                    for i in range(len(global_perem_spisok)):
                        if (global_perem_spisok[i].get_first() != 0 and global_perem_spisok[i].get_last() != 0):
                            print(str(global_perem_spisok[i].get_name()) + " = " + str(global_perem_spisok[i].get_value()))
                        else:
        ...
        if not char.isspace():
            if char.isdigit(): # Символ является числом
                if not '0' <= char <= '7':
                    ...
                    showerror('Ошибка_1', 'Встречен неверный символ: '+ char + ' ! ' + str(j)+"."+str(i) + ' ' + str(j)+"."+str(i+1) )
                    reset_to_defolt()
                    return False
            elif char.isalpha(): # Символ является буквой
                if not 'А' <= char <= 'я':
                    text.tag_add("highlightline", str(j)+"."+str(i), str(j)+"."+str(i+1))
                    showerror('Ошибка_2', 'Встречен неверный символ: '+ char + ' ! ' + str(j)+"."+str(i) + ' ' + str(j)+"."+str(i+1) )
                    reset_to_defolt()
                    return False

            else: # Символ не является операндом
                if not (char == '+' or char == '-' or char == '*' or char == '/' or char == '&' or char == '|' or char == ')' or char == '(' or char == ':' or char == ',' or char == '.' or char == '='):
                    text.tag_add("highlightline", str(j)+"."+str(i), str(j)+"."+str(i+1))
                    showerror('Ошибка_3', 'Встречен неверный символ: '+ char + ' ! ' + str(j)+"."+str(i) + ' ' + str(j)+"."+str(i+1) )
                    reset_to_defolt()
                    return False
            i += 1
    return True
        ...
        if splited_str[global_i_in_splited_str] == "Переменные:":
            ...
            while True:
                ...
                if not is_perem(splited_str[global_i_in_splited_str], 1):
                    if (splited_str[global_i_in_splited_str] == "Переменные:" or splited_str[global_i_in_splited_str] == "Метки:" ) and indikator != 0:
                        #update_global_i_j_in_file(-len(splited_str[global_i_in_splited_str]))
                        global_i_in_splited_str -= 1
                        break
                    elif (splited_str[global_i_in_splited_str] == "Переменные:" or splited_str[global_i_in_splited_str] == "Метки:" ) and indikator == 0:
                        update_global_i_j_in_file(-len(splited_str[global_i_in_splited_str]))
                        global_i_in_splited_str -= 1
                        text.tag_add("highlightline", str(global_j_in_file) + "." + str(global_i_in_file),str(global_j_in_file) + "." + str(global_i_in_file + len(splited_str[global_i_in_splited_str])))
                        showerror("Ошибка 3.12","Должна быть хотябы одна переменная" + splited_str[global_i_in_splited_str + 1])
                        return False
                        ...
                        update_global_i_j_in_file(-len(splited_str[global_i_in_splited_str]))
                        text.tag_add("highlightline", str(global_j_in_file) + "." + str(global_i_in_file),str(global_j_in_file) + "." + str(global_i_in_file + len(splited_str[global_i_in_splited_str])))
                        showerror("Ошибка 3.3","Ожидалась переменная. Полученно: " + splited_str[global_i_in_splited_str])
                        return False
                else:
                    indikator += 1
            ...
            while True:
                ...
                if not is_integer(splited_str[global_i_in_splited_str]):
                    if splited_str[global_i_in_splited_str] == "Конец" and splited_str[global_i_in_splited_str+1] == "меток":
                        update_global_i_j_in_file(len(splited_str[global_i_in_splited_str]))
                        global_i_in_splited_str += 1
                        #update_global_i_j_in_file(len(splited_str[global_i_in_splited_str]))
                        print("ТУТ")
                        ...
                        update_global_i_j_in_file(-len(splited_str[global_i_in_splited_str]))
                        text.tag_add("highlightline", str(global_j_in_file) + "." + str(global_i_in_file),str(global_j_in_file) + "." + str(global_i_in_file + len(splited_str[global_i_in_splited_str])))
                        showerror("Ошибка 3.10", "Ожидалось целое число: Полученно: " + splited_str[global_i_in_splited_str])
                        return False
                else:
                    w = 0
            ...
            print(f"i = {global_i_in_splited_str}")
            text.tag_add("highlightline", str(global_j_in_file) + "." + str(global_i_in_file), str(global_j_in_file) + "." + str(global_i_in_file + len(splited_str[global_i_in_splited_str])))
            showerror("Ошибка 3.5","Ожидалось: <<Переменные:>> или <<Метки:>>." + '\n' + "Получено: " + splited_str[global_i_in_splited_str])
            return False

        if (splited_str[global_i_in_splited_str+1] != "Переменные:" and splited_str[global_i_in_splited_str+1] != "Метки:") and (is_integer(splited_str[global_i_in_splited_str+1] and indikator != 0)):
            break
    return True
...
def find_perem(splited_str):
    ...
    for i in range(len(global_perem_spisok)):
        print(f"Проверка: Perem[{i}].get_name() = {str(global_perem_spisok[i].get_name())} Переменная в проге = {splited_str[global_i_in_splited_str]}")
...
def find_end_operator(splited_str):
    ...
    while global_i_in_splited_str+1 < len(splited_str) and splited_str[global_i_in_splited_str+1] != "Конец" and splited_str[global_i_in_splited_str+1] != ',':
        print(f"Поиск <<,>> или <<Конец>>: str = {splited_str[global_i_in_splited_str]} global_i_in_splited_str = {global_i_in_splited_str}")
        update_global_i_j_in_file(len(splited_str[global_i_in_splited_str]))
        global_i_in_splited_str += 1
    print(f"Поиск закончен: global_i_in_splited_srt = {global_i_in_splited_str}, len(splited_str) = {len(splited_str)}, слово = {splited_str[global_i_in_splited_str]}")
        ...
        if skip_metki(splited_str): # Остановка на :
            ...
            if find_perem(splited_str): # Переменная объявлена
                ...
                if splited_str[global_i_in_splited_str] == '=':
                    update_global_i_j_in_file(len(splited_str[global_i_in_splited_str]))
                    global_i_in_splited_str += 1
                    if splited_str[global_i_in_splited_str] == "Конец" or splited_str[global_i_in_splited_str] == ',' or len(splited_str) <= global_i_in_splited_str:
                    ...
                    if find_end_operator(splited_str):
                        ...
                        print(f"first = {global_perem_spisok[global_perem_num].get_first()} last = {global_perem_spisok[global_perem_num].get_last()}")
                        if not global_perem_spisok[global_perem_num].get_first() == global_perem_spisok[global_perem_num].get_last():
                            print("-----Не равны-----")
                            global_perem_spisok[global_perem_num].set_example()
                            if splited_str[global_i_in_splited_str + 1] == "Конец":
                                return True
                            elif splited_str[global_i_in_splited_str] == "," and global_i_in_splited_str == len(splited_str) - 1:
                                text.tag_add("highlightline", str(global_j_in_file) + "." + str(global_i_in_file),str(global_j_in_file) + "." + str(global_i_in_file + len(splited_str[global_i_in_splited_str])))
